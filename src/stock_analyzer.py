...
"""
===================================
美股 ETF 趋势交易分析器 - 拥抱长牛理念
===================================

交易理念核心原则：
1. 顺势而为 - 拥抱美股长牛，只要均线多头排列就以持有/做多为主
2. 趋势交易 - MA5>MA10>MA20 多头排列是核心前提
3. 动量优先 - 关注突破放量与强势趋势，不再纠结于A股的“庄家洗盘”
4. 买点偏好 - 强势拉升跟进，或在 MA5/MA10 附近获得支撑时买入

技术标准：
- 多头排列：MA5 > MA10 > MA20
- 乖离率：(Close - MA5) / MA5 < 8%（容忍美股财报/宏观数据带来的跳空强势）
- 量能形态：放量上涨最佳，温和缩量回调次之
"""
...
class StockTrendAnalyzer:
    ...
    基于美股宏观长牛理念实现：
    1. 趋势判断 - MA5>MA10>MA20 多头排列（美股核心引擎）
    2. 乖离率检测 - 偏离 MA5 超过 8% 视为短期超买，注意均线回归风险
    3. 量能分析 - 偏好放量突破/上涨（真实资金推动）
    4. 买点识别 - 强势跟随，或回踩 MA5/MA10 支撑
    5. MACD 指标 - 趋势确认和金叉死叉信号
    6. RSI 指标 - 超买超卖判断
    """
    
    # 交易参数配置
    BIAS_THRESHOLD = 8.0        # 乖离率阈值（%），超过此值不买入
    VOLUME_SHRINK_RATIO = 0.85   # 缩量判断阈值（当日量/5日均量）
    VOLUME_HEAVY_RATIO = 1.3    # 放量判断阈值
